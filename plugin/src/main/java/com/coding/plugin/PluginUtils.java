...
public final class PluginUtils {
    ...
    public static Context getTopActivityOrApp() {
        if (isAppForeground()) {
            ...
            return topActivity == null ? PluginUtils.getApp() : topActivity;
        } else {
            return PluginUtils.getApp();
        }
    }

    public static boolean isAppForeground() {
        ActivityManager am =
                (ActivityManager) PluginUtils.getApp().getSystemService(Context.ACTIVITY_SERVICE);
        //noinspection ConstantConditions
        List<ActivityManager.RunningAppProcessInfo> info = am.getRunningAppProcesses();
        if (info == null || info.size() == 0) return false;
        for (ActivityManager.RunningAppProcessInfo aInfo : info) {
            if (aInfo.importance == ActivityManager.RunningAppProcessInfo.IMPORTANCE_FOREGROUND) {
                return aInfo.processName.equals(PluginUtils.getApp().getPackageName());
            }
        }
        return false;
    }
    ...
    public static void restoreAdaptScreen() {
        ...
        final DisplayMetrics appDm = PluginUtils.getApp().getResources().getDisplayMetrics();
